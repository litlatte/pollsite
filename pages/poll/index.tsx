...
export default function PollPage({token}: any){
    ...
    return <div className="h-full py-4 w-full flex flex-col items-center justify-center bg-indigo-50">
            ...
            <input value={question} onChange={(e)=>{
             ...
             {answers.map((ans, i)=>{
             return <div className="flex my-2 items-center mx-auto w-fit">
                <div className="rounded-full mr-2 shadow bg-white w-8 h-8 flex items-center justify-center">{i}</div> <input value={ans.answer.toString()} onChange={
                    (e)=>{
                        setAnswers([...answers.slice(0, i), {answer: e.target.value}, ...answers.slice(i+1)])
                    }
            ...
            {errorMessage&&<ErrorMessageCard className="mt-3">{errorMessage}</ErrorMessageCard>}
            <button onClick={handleSubmit} className="bg-indigo-700 text-white hover:opacity-80 hover:scale-110 transition duration-300 shadow p-3 rounded-lg mt-4">
                Submit
            </button>
        </div>
}
