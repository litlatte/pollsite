  ...
  return (
    <div className="w-full bg-indigo-50 h-full flex flex-col items-center justify-center">
      ...
      <div className="w-fit h-fit">
        ...
        <div className="flex flex-col h-72 grow overflow-auto">
          {poll.options.map((op: any) => (
            <div
              className={` rounded-xl shadow p-6 py-3 mt-2 ${
                poll.totalVotes
                  ? op.id === mostVotedAnswer.id
                    ? "bg-green-100"
                    : op.id == leastVotedAnswer.id ||
                      op.votes == leastVotedAnswer.votes
                    ? "bg-red-100"
                    : "bg-white"
                  : "bg-white"
              } grid grid-cols-6`}
      ...
      <div className="flex mt-2 items-center justify-center p-2 bg-white mt-3 rounded-xl shadow">
        <div className="mr-2 max-w-[250px] text-indigo-900 truncate">{url}</div>
        <Image
...
export async function getServerSideProps(context: any) {
  ...
  if (res.status !== 200) {
    ...
    return {
      props: {
        url: `${process.env.NEXTAUTH_URL}/poll/${context.query.slug}`,
        poll: data,
      },
    }
  }
