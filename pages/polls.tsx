...
export default function PollsPage({ token, polls }: any) {
    return <div className="w-full h-full bg-gray-50 justify-center">
        <h1 className="text-5xl text-indigo-700 text-center pb-10 pt-16">Your Polls</h1>
        <div className="h-80 w-fit px-8 mx-auto grow overflow-auto">
            {polls.map((poll: any) => {
                return <div className="w-fit min-w-[300px] mx-auto flex flex-col items-center justify-center bg-white text-black text-xl mt-3 transition duration-300 p-4 rounded-xl shadow">
                {poll.question}
                    <Link href={`/poll/${poll.slug}/`}>
                        <a className="text-lg text-indigo-800">
                            Poll
                        </a>
                    </Link>
